...
var sqlite_migrations = require('./sqlite_migrations.js');
...
if (bCordova){
	...
	path = require('../base/desktop_app.js'+'').getAppDataDir() + '/';
		...
		fs.stat(path + db_name, function(err, stats){
			...
			fs.mkdir(parent_dir, mode, function(err){
				...
				fs.mkdir(path, mode, function(err){
					...
					fs.createReadStream(__dirname + '/db/' + initial_db_filename).pipe(fs.createWriteStream(path + db_name)).on('finish', onDbReady);
				});
			});
		});
	}
